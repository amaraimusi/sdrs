...
class WorkCategory extends AppModel {
	...
	public $useTable = 'work_categorys';
	 ...
	 * 作業カテゴリエンティティを取得
	 *
	 * 作業カテゴリテーブルからidに紐づくエンティティを取得します。
	 *
	 * @param int $id 作業カテゴリID
	 * @return array 作業カテゴリエンティティ
	 */
	...
	private function dumpSql($option){
		...
		$option['table']=$dbo->fullTableName($this->WorkCategory);
		$option['alias']='WorkCategory';
		
		$query = $dbo->buildStatement($option,$this->WorkCategory);
	...
	private function createKjConditions($kjs){
		...
		if(!empty($kjs['kj_main'])){
			$cnds[]="CONCAT( IFNULL(WorkCategory.work_category_name, '') ,IFNULL(WorkCategory.note, '')) LIKE '%{$kjs['kj_main']}%'";
		}
		
		// CBBXS-1003
		if(!empty($kjs['kj_id']) || $kjs['kj_id'] ==='0' || $kjs['kj_id'] ===0){
			$cnds[]="WorkCategory.id = {$kjs['kj_id']}";
		}
		if(!empty($kjs['kj_work_category_name'])){
			$cnds[]="WorkCategory.work_category_name LIKE '%{$kjs['kj_work_category_name']}%'";
		}
		if(!empty($kjs['kj_note'])){
			$cnds[]="WorkCategory.note LIKE '%{$kjs['kj_note']}%'";
		}
		if(!empty($kjs['kj_sort_no']) || $kjs['kj_sort_no'] ==='0' || $kjs['kj_sort_no'] ===0){
			$cnds[]="WorkCategory.sort_no = {$kjs['kj_sort_no']}";
		}
		$kj_delete_flg = $kjs['kj_delete_flg'];
		if(!empty($kjs['kj_delete_flg']) || $kjs['kj_delete_flg'] ==='0' || $kjs['kj_delete_flg'] ===0){
			if($kjs['kj_delete_flg'] != -1){
			   $cnds[]="WorkCategory.delete_flg = {$kjs['kj_delete_flg']}";
			}
		}
		if(!empty($kjs['kj_update_user'])){
			$cnds[]="WorkCategory.update_user LIKE '%{$kjs['kj_update_user']}%'";
		}
		if(!empty($kjs['kj_ip_addr'])){
			$cnds[]="WorkCategory.ip_addr LIKE '%{$kjs['kj_ip_addr']}%'";
		}
		if(!empty($kjs['kj_created'])){
			$kj_created=$kjs['kj_created'].' 00:00:00';
			$cnds[]="WorkCategory.created >= '{$kj_created}'";
		}
		if(!empty($kjs['kj_modified'])){
			$kj_modified=$kjs['kj_modified'].' 00:00:00';
			$cnds[]="WorkCategory.modified >= '{$kj_modified}'";
		}
	 ...
	 * エンティティをDB保存
	 *
	 * 作業カテゴリエンティティを作業カテゴリテーブルに保存します。
	 *
	 * @param array $ent 作業カテゴリエンティティ
	 * @param array $option オプション
	 *  - form_type フォーム種別  new_inp:新規入力 , copy:複製 , edit:編集
	 *  - ni_tr_place 新規入力追加場所フラグ 0:末尾 , 1:先頭
	 * @return array 作業カテゴリエンティティ（saveメソッドのレスポンス）
	 */
	public function saveEntity($ent,$option=array()){
		...
		$ent = $this->find('first',
				array(
						'conditions' => "id={$ent['WorkCategory']['id']}"
				));

		$ent=$ent['WorkCategory'];
