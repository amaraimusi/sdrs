...
$(function() {
	...
	$('#daily_report_tbl').show();// 高速表示のためテーブルは最後に表示する
...
function init(){
	...
	crudBase = new CrudBase({
			'src_code':'daily_report', // 画面コード（スネーク記法)
			'kjs':kjs,
			'ni_tr_place':1,
		});
	...
	// CBBXS-1023
	// 現場リストJSON
	var site_id_json = jQuery('#site_id_json').val();
	var siteIdList = JSON.parse(site_id_json);
	disFilData['site_id'] ={'fil_type':'select','option':{'list':siteIdList}};
	// 作業カテゴリリストJSON
	var work_category_id_json = jQuery('#work_category_id_json').val();
	var workCategoryIdList = JSON.parse(work_category_id_json);
	disFilData['work_category_id'] ={'fil_type':'select','option':{'list':workCategoryIdList}};
	...
	pwms = new ProcessWithMultiSelection({
		'tbl_slt':'#daily_report_tbl',
		'ajax_url':'daily_report/ajax_pwms',
			});
	...
	crudBase.crudBaseBulkAdd.init(
		[
			{'field':'work_date', 'inp_type':'textarea'}, 
			{'field':'site_id', 'inp_type':'select', 'list':siteIdList}, 
			{'field':'work_note', 'inp_type':'textarea'}, 
			{'field':'work_category_id', 'inp_type':'select', 'list':workCategoryIdList}, 
		],
		{
			'ajax_url':'daily_report/bulk_reg',
			'ta_placeholder':"Excelからコピーした作業日付、作業内容を貼り付けてください。（タブ区切りテキスト）\n(例)\n2019-1-17\t開発作業\n2019-1-18\tリリース作業\n",
		}
	);
}
...
function moveClmSorter(){
	...
	moveClmSorterBase('daily_report');
...
function calendarViewKShow(){
	// カレンダービューを生成 
	crudBase.calendarViewCreate('daily_report_date');
}
