 ...
 * 
 * @note
 * 現場画面では現場一覧を検索閲覧、編集など多くのことができます。
 * 
...
class SiteController extends CrudBaseController {
	...
	public $uses = array('Site','CrudBase');
	
	/// オリジナルヘルパーの登録[CakePHPの機能]
	public $helpers = array('CrudBase');

	/// デフォルトの並び替え対象フィールド
	public $defSortFeild='Site.sort_no';
	...
	//public $viaDpFnMap = array('img_fn'=>'site_group');
...
// 		}
	...
	public function index() {
		...
		// CrudBase共通処理（前）
		$crudBaseData = $this->indexBefore('Site');//indexアクションの共通先処理(CrudBaseController)
		
		//一覧データを取得
		$data = $this->Site->findData($crudBaseData);
	...
	public function front_a(){
		...
		// フロントA用のコンポーネント
		$this->SiteFrontA = $this->Components->load('SiteFrontA');
		...
		$crudBaseData = $this->indexBefore('Site',$option);//indexアクションの共通先処理(CrudBaseController)
		
		// ディレクトリパステンプレートを調整する(パスはindex用の相対パスになっているのでズレを調整しなければならない）
		$crudBaseData['dp_tmpl'] = $this->SiteFrontA->adjustDpt($crudBaseData['dp_tmpl']);
...
// 		}
		...
		// エンティティをDB保存
		$this->Site->begin();
		$ent = $this->Site->saveEntity($ent,$regParam);
		$this->Site->commit();//コミット
	...
	public function ajax_delete(){
		...
		$this->Site->begin();
		if($eliminate_flg == 0){
			$ent = $this->Site->saveEntity($ent,$regParam); // 更新
		}else{
			$this->Site->eliminateFiles($ent['id'], 'img_fn', $ent, $this->dp_tmpl, $this->viaDpFnMap); // ファイル抹消（他のレコードが保持しているファイルは抹消対象外）
			$this->Site->delete($ent['id']); // 削除
		}
		$this->Site->commit();//コミット
	...
	*/
	public function auto_save(){
		...
		// データ保存
		$this->Site->begin();
		$this->Site->saveAll($data); // まとめて保存。内部でSQLサニタイズされる。
		$this->Site->commit();
	...
	public function csv_fu(){
		...
		$this->csv_fu_base($this->Site,array('id','site_val','site_name','site_date','site_group','site_dt','site_flg','img_fn','note','sort_no'));
	...
	private function getDataForDownload(){
		...
		//セッションから検索条件情報を取得
		$kjs=$this->Session->read('site_kjs');
		
		// セッションからページネーション情報を取得
		$pages = $this->Session->read('site_pages');
	...
	private function initCrudBase(){
		...
		$this->kensakuJoken=array(
			...
			array('name'=>'kj_site_name','def'=>null),
		...
		$this->field_data = array('def'=>array(
			...
			'id'=>array(
					'name'=>'ID',//HTMLテーブルの列名
					'row_order'=>'Site.id',//SQLでの並び替えコード
					'clm_show'=>1,//デフォルト列表示 0:非表示 1:表示
			),
			'site_name'=>array(
					'name'=>'現場名',
					'row_order'=>'Site.site_name',
					'clm_show'=>1,
			),
			'note'=>array(
					'name'=>'備考',
			...
			'sort_no'=>array(
					'name'=>'順番',
					'row_order'=>'Site.sort_no',
					'clm_show'=>0,
			),
			'delete_flg'=>array(
					'name'=>'無効フラグ',
					'row_order'=>'Site.delete_flg',
					'clm_show'=>0,
			),
			'update_user'=>array(
					'name'=>'更新者',
					'row_order'=>'Site.update_user',
					'clm_show'=>0,
			),
			'ip_addr'=>array(
					'name'=>'IPアドレス',
					'row_order'=>'Site.ip_addr',
					'clm_show'=>0,
			),
			'created'=>array(
					'name'=>'生成日時',
					'row_order'=>'Site.created',
					'clm_show'=>0,
			),
			'modified'=>array(
					'name'=>'更新日',
					'row_order'=>'Site.modified',
					'clm_show'=>0,
			),
