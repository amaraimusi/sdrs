 ...
 * 
 * @note
 * 日報画面では日報一覧を検索閲覧、編集など多くのことができます。
 * 
 * @date 2015-9-16 | 2018-10-10
 * @version 3.2.2
 *
 */
class DailyReportController extends CrudBaseController {

	/// 名称コード
	public $name = 'DailyReport';
	
	/// 使用しているモデル[CakePHPの機能]
	public $uses = array('DailyReport','CrudBase');
	
	/// オリジナルヘルパーの登録[CakePHPの機能]
	public $helpers = array('CrudBase');

	/// デフォルトの並び替え対象フィールド
	public $defSortFeild='DailyReport.sort_no';
	...
	//public $viaDpFnMap = array('img_fn'=>'daily_report_group');
...
// 		}
	...
	public function index() {
		...
		// CrudBase共通処理（前）
		$crudBaseData = $this->indexBefore('DailyReport');//indexアクションの共通先処理(CrudBaseController)
		
		//一覧データを取得
		$data = $this->DailyReport->findData($crudBaseData);
		...
		// 現場リスト
		$siteIdList = $this->DailyReport->getSiteIdList();
		$site_id_json = json_encode($siteIdList,JSON_HEX_TAG | JSON_HEX_QUOT | JSON_HEX_AMP | JSON_HEX_APOS);
		$this->set(array('siteIdList' => $siteIdList,'site_id_json' => $site_id_json));

		// 作業カテゴリリスト
		$workCategoryIdList = $this->DailyReport->getWorkCategoryIdList();
		$work_category_id_json = json_encode($workCategoryIdList,JSON_HEX_TAG | JSON_HEX_QUOT | JSON_HEX_AMP | JSON_HEX_APOS);
		$this->set(array('workCategoryIdList' => $workCategoryIdList,'work_category_id_json' => $work_category_id_json));
	...
	public function front_a(){
		...
		// フロントA用のコンポーネント
		$this->DailyReportFrontA = $this->Components->load('DailyReportFrontA');
		...
		$crudBaseData = $this->indexBefore('DailyReport',$option);//indexアクションの共通先処理(CrudBaseController)
		
		// ディレクトリパステンプレートを調整する(パスはindex用の相対パスになっているのでズレを調整しなければならない）
		$crudBaseData['dp_tmpl'] = $this->DailyReportFrontA->adjustDpt($crudBaseData['dp_tmpl']);
...
// 		}
		...
		// エンティティをDB保存
		$this->DailyReport->begin();
		$ent = $this->DailyReport->saveEntity($ent,$regParam);
		$this->DailyReport->commit();//コミット
	...
	public function ajax_delete(){
		...
		$this->DailyReport->begin();
		if($eliminate_flg == 0){
			$ent = $this->DailyReport->saveEntity($ent,$regParam); // 更新
		}else{
			$this->DailyReport->eliminateFiles($ent['id'], 'img_fn', $ent, $this->dp_tmpl, $this->viaDpFnMap); // ファイル抹消（他のレコードが保持しているファイルは抹消対象外）
			$this->DailyReport->delete($ent['id']); // 削除
		}
		$this->DailyReport->commit();//コミット
	...
	*/
	public function auto_save(){
		...
		// データ保存
		$this->DailyReport->begin();
		$this->DailyReport->saveAll($data); // まとめて保存。内部でSQLサニタイズされる。
		$this->DailyReport->commit();
	...
	public function bulk_reg(){
		...
		$res = $bulkReg->reg('daily_reports', $param);
	...
	public function csv_fu(){
		...
		$this->csv_fu_base($this->DailyReport,array('id','daily_report_val','daily_report_name','daily_report_date','daily_report_group','daily_report_dt','daily_report_flg','img_fn','note','sort_no'));
	...
	private function getDataForDownload(){
		...
		//セッションから検索条件情報を取得
		$kjs=$this->Session->read('daily_report_kjs');
		
		// セッションからページネーション情報を取得
		$pages = $this->Session->read('daily_report_pages');
	...
	private function initCrudBase(){
		...
		$this->kensakuJoken=array(
			...
			array('name'=>'kj_work_date','def'=>null),
			array('name'=>'kj_site_id','def'=>null),
			array('name'=>'kj_site_img_fn','def'=>null),
			array('name'=>'kj_work_note','def'=>null),
			array('name'=>'kj_work_category_id','def'=>null),
			array('name'=>'kj_check_flg','def'=>null),
		...
		$this->kjs_validate=array(
				...
				'kj_site_id' => array(
						'custom'=>array(
								...
								'message' => '現場は整数を入力してください。',
								'allowEmpty' => true
						),
				),
				'kj_site_img_fn'=> array(
						'maxLength'=>array(
								'rule' => array('maxLength', 255),
								'message' => '現場写真は255文字以内で入力してください',
								'allowEmpty' => true
						),
				),
				'kj_work_note'=> array(
						'maxLength'=>array(
								'rule' => array('maxLength', 255),
								'message' => '作業内容は0文字以内で入力してください',
								'allowEmpty' => true
						),
				),
				'kj_work_category_id' => array(
						'custom'=>array(
								'rule' => array( 'custom', '/^[-]?[0-9]+$/' ),
								'message' => '作業カテゴリは整数を入力してください。',
								'allowEmpty' => true
						),
				),
				'kj_sort_no' => array(
		...
		$this->field_data = array('def'=>array(
			...
			'id'=>array(
					'name'=>'ID',//HTMLテーブルの列名
					'row_order'=>'DailyReport.id',//SQLでの並び替えコード
					'clm_show'=>1,//デフォルト列表示 0:非表示 1:表示
			),
			'work_date'=>array(
					'name'=>'作業日',
					'row_order'=>'DailyReport.work_date',
					'clm_show'=>1,
			),
			'site_id'=>array(
					'name'=>'現場',
					'row_order'=>'DailyReport.site_id',
					'clm_show'=>1,
			),
			'site_img_fn'=>array(
					'name'=>'現場写真',
					'row_order'=>'DailyReport.site_img_fn',
					'clm_show'=>1,
			),
			'work_note'=>array(
					'name'=>'作業内容',
					'row_order'=>'DailyReport.work_note',
					'clm_show'=>1,
			),
			'work_category_id'=>array(
					'name'=>'作業カテゴリ',
					'row_order'=>'DailyReport.work_category_id',
					'clm_show'=>1,
			),
			'check_flg'=>array(
					'name'=>'確認フラグ',
					'row_order'=>'DailyReport.check_flg',
					'clm_show'=>1,
			),
			'sort_no'=>array(
					'name'=>'順番',
					'row_order'=>'DailyReport.sort_no',
					'clm_show'=>0,
			),
			'delete_flg'=>array(
					'name'=>'無効フラグ',
					'row_order'=>'DailyReport.delete_flg',
					'clm_show'=>0,
			),
			'update_user'=>array(
					'name'=>'更新者',
					'row_order'=>'DailyReport.update_user',
					'clm_show'=>0,
			),
			'ip_addr'=>array(
					'name'=>'IPアドレス',
					'row_order'=>'DailyReport.ip_addr',
					'clm_show'=>0,
			),
			'created'=>array(
					'name'=>'生成日時',
					'row_order'=>'DailyReport.created',
					'clm_show'=>0,
			),
			'modified'=>array(
					'name'=>'更新日',
					'row_order'=>'DailyReport.modified',
					'clm_show'=>0,
			),
