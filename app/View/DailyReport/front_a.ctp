...
<!-- 一覧テーブル -->
<table id="daily_report_tbl" border="1"  class="table_transform">
...
foreach($data as $i=>$ent){
	...
	$this->FrontA->tdPlain($ent,'work_date');
	$this->FrontA->tdPlain($ent,'site_id');
	$this->FrontA->tdImage($ent,'site_img_fn','td_image');
	$this->FrontA->tdNote($ent,'work_note','td_note');
	$this->FrontA->tdPlain($ent,'work_category_id');
	$this->FrontA->tdPlain($ent,'check_flg');
