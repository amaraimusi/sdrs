<?php
$this->CrudBase->init(array('model_name'=>'DailyReport'));

// CSSファイルのインクルード
$cssList = $this->CrudBase->getCssList();
$cssList[] = 'DailyReport/index'; // 当画面専用CSS
$this->assign('css', $this->Html->css($cssList));

// JSファイルのインクルード
$jsList = $this->CrudBase->getJsList();
$jsList[] = 'DailyReport/index'; // 当画面専用JavaScript
...
<div class="cb_func_line">
	...
	<div class="cb_kj_main">
		<!-- 検索条件入力フォーム -->
		<?php echo $this->Form->create('DailyReport', array('url' => true , 'class' => 'form_kjs')); ?>
		<?php $this->CrudBase->inputKjMain($kjs,'kj_main','',null,'作業日、作業内容を検索する');?>
		...
		$this->CrudBase->inputKjText($kjs,'kj_work_date','作業日');
		$this->CrudBase->inputKjSelect($kjs,'kj_site_id','現場',$siteIdList); 
		$this->CrudBase->inputKjText($kjs,'kj_site_img_fn','現場写真');
		$this->CrudBase->inputKjText($kjs,'kj_work_note','作業内容');
		$this->CrudBase->inputKjSelect($kjs,'kj_work_category_id','作業カテゴリ',$workCategoryIdList); 
		$this->CrudBase->inputKjFlg($kjs,'kj_check_flg','確認フラグ');
...
<div id="detail_div" style="display:none">
	...
	<div id="main_tools" style="margin-bottom:10px;">
		<?php 
			...
			// CSVエクスポート機能
			$csv_dl_url = $this->html->webroot . 'daily_report/csv_download';
	...
	<div id="sub_tools">
		...
		<a href="daily_report/front_a?<?php echo $pages['query_str']; ?>" class="btn btn-default btn-xs" target="brank" >フロント画面表示</a>
	</div><!-- sub_tools -->
</div><!-- detail_div -->
...
<!-- 一覧テーブル -->
<table id="daily_report_tbl" border="1"  class="table table-striped table-bordered table-condensed" style="display:none;margin-bottom:0px">
...
foreach($data as $i=>$ent){
	...
	$this->CrudBase->tdPlain($ent,'work_date');
	$this->CrudBase->tdList($ent,'site_id',$siteIdList);
	$this->CrudBase->tdImage($ent,'site_img_fn');
	$this->CrudBase->tdNote($ent,'work_note');
	$this->CrudBase->tdList($ent,'work_category_id',$workCategoryIdList);
	$this->CrudBase->tdFlg($ent,'check_flg');
...
<table id="crud_base_forms">
	...
	<tr id="ajax_crud_new_inp_form" class="crud_base_form" style="display:none;padding-bottom:60px"><td colspan='5'>
		...
		<div class="cbf_inp_wrap">
			<div class='cbf_inp_label' >作業日: </div>
			<div class='cbf_input'>
				<input type="text" name="work_date" class="valid datepicker" value=""  pattern="([0-9]{4})(/|-)([0-9]{1,2})(/|-)([0-9]{1,2})" title="日付形式（Y-m-d）で入力してください(例：2012-12-12)" />
				<label class="text-danger" for="work_date"></label>
			</div>
		</div>
		<div class="cbf_inp_wrap">
			<div class='cbf_inp_label' >現場: </div>
			<div class='cbf_input'>
				<?php $this->CrudBase->selectX('site_id',null,$siteIdList,null);?>
				<label class="text-danger" for="site_id"></label>
			</div>
		</div>

		<div class="cbf_inp_wrap">
			<div class='cbf_inp_label_long' >現場写真: </div>
			<div class='cbf_input'>
				<label for="site_img_fn_n" class="fuk_label" style="width:100px;height:100px;">
					<input type="file" id="site_img_fn_n" class="site_img_fn" style="display:none" accept="image/*" title="画像ファイルのアップロード" />
				</label>
			</div>
		</div>

		<div class="cbf_inp_wrap_long">
			<div class='cbf_inp_label' >作業内容： </div>
			<div class='cbf_input'>
				<textarea name="work_note" maxlength="1000" title="1000文字以内で入力してください" style="height:100px;width:100%"></textarea>
				<label class="text-danger" for="work_note"></label>
			</div>
		</div>
		<div class="cbf_inp_wrap">
			<div class='cbf_inp_label' >作業カテゴリ: </div>
			<div class='cbf_input'>
				<?php $this->CrudBase->selectX('work_category_id',null,$workCategoryIdList,null);?>
				<label class="text-danger" for="work_category_id"></label>
			</div>
		</div>

		<div class="cbf_inp_wrap">
			<div class='cbf_inp_label' >確認フラグ: </div>
			<div class='cbf_input'>
				<input type="checkbox" name="check_flg" class="valid"/>
				<label class="text-danger" for="check_flg" ></label>
			</div>
		</div>
	...
	<tr id="ajax_crud_edit_form" class="crud_base_form" style="display:none"><td colspan='5'>
		...
		<div class="cbf_inp_wrap">
			<div class='cbf_inp_label_long' >現場写真: </div>
			<div class='cbf_input'>
				<label for="site_img_fn_e" class="fuk_label" style="width:100px;height:100px;">
					<input type="file" id="site_img_fn_e" class="site_img_fn" style="display:none" accept="image/*" title="画像ファイルのアップロード" />
				</label>
			</div>
		</div>
...
<div id="ajax_crud_delete_form" class="panel panel-danger" style="display:none">
	...
	<table><tbody>
		...
		<tr><td>日報名: </td><td>
			<span class="daily_report_name"></span>
		</td></tr>
...
<div style="display:none">
	...
	<!-- CBBXS-1022 -->
	<input id="site_id_json" type="hidden" value='<?php echo $site_id_json; ?>' />
	<input id="work_category_id_json" type="hidden" value='<?php echo $work_category_id_json; ?>' />
