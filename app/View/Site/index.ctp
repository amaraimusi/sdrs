<?php
$this->CrudBase->init(array('model_name'=>'Site'));

// CSSファイルのインクルード
$cssList = $this->CrudBase->getCssList();
$cssList[] = 'Site/index'; // 当画面専用CSS
$this->assign('css', $this->Html->css($cssList));

// JSファイルのインクルード
$jsList = $this->CrudBase->getJsList();
$jsList[] = 'Site/index'; // 当画面専用JavaScript
...
<div class="cb_func_line">
	...
	<div class="cb_kj_main">
		<!-- 検索条件入力フォーム -->
		<?php echo $this->Form->create('Site', array('url' => true , 'class' => 'form_kjs')); ?>
		<?php $this->CrudBase->inputKjMain($kjs,'kj_main','',null,'現場名、備考を検索する');?>
		...
		$this->CrudBase->inputKjText($kjs,'kj_site_name','現場名');
...
<div id="detail_div" style="display:none">
	...
	<div id="main_tools" style="margin-bottom:10px;">
		<?php 
			...
			// CSVエクスポート機能
			$csv_dl_url = $this->html->webroot . 'site/csv_download';
	...
	<div id="sub_tools">
		...
		<a href="site/front_a?<?php echo $pages['query_str']; ?>" class="btn btn-default btn-xs" target="brank" >フロント画面表示</a>
	</div><!-- sub_tools -->
</div><!-- detail_div -->
...
<!-- 一覧テーブル -->
<table id="site_tbl" border="1"  class="table table-striped table-bordered table-condensed" style="display:none;margin-bottom:0px">
...
foreach($data as $i=>$ent){
	...
	$this->CrudBase->tdStr($ent,'site_name');
...
<table id="crud_base_forms">
	...
	<tr id="ajax_crud_new_inp_form" class="crud_base_form" style="display:none;padding-bottom:60px"><td colspan='5'>
		...
		<div class="cbf_inp_wrap">
			<div class='cbf_inp' >現場名: </div>
			<div class='cbf_input'>
				<input type="text" name="site_name" class="valid " value=""  maxlength="255" title="255文字以内で入力してください" />
				<label class="text-danger" for="site_name"></label>
			</div>
		</div>
